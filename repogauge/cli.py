...
from repogauge.runner.progress import emit_progress_line
    ...
    for name in ["mine", "review", "export", "eval", "run", "analyze", "train-router"]:
        ...
        if name == "eval":
            ...
            cmd.add_argument(
                ...
                default=4,
                type=int,
                help="Max parallel containerized instance evaluations.",
            )
            cmd.add_argument(
                "--container-runtime",
        ...
        if name == "analyze":
            ...
            cmd.add_argument(
                ...
                action="store_true",
                help=(
                    "Do not auto-run evaluation. Requires instance_results.jsonl"
            ...
            cmd.add_argument(
                ...
                default=4,
                type=int,
                help="Max parallel containerized instance evaluations for auto eval.",
            )
            cmd.add_argument(
                "--container-runtime",
                choices=["docker", "podman"],
...
def _inputs_hash(command: str, namespace: argparse.Namespace) -> str:
    ...
    payload = {
        ...
        "dataset": namespace.dataset if command in {"run", "analyze"} else "",
        "run_id": namespace.run_id if command == "run" else "",
        "jobs": getattr(namespace, "jobs", 0),
        "retry_failures_from": (
    ...
    *,
    ...
    if adapter_path is not None:
        ...
        if specs_path.exists():
            adapter_spec = json.loads(specs_path.read_text(encoding="utf-8"))

    emit_progress_line("repogauge eval", "using shared containerized evaluator")
    with ensure_container_runtime(
        container_runtime=container_runtime,
        container_host=container_host,
        log_prefix="repogauge eval",
...
def _run_command(namespace: argparse.Namespace) -> int:
    ...
    if command == "eval":
        ...
        if adapter_path is not None:
            ...
            print(
                "repogauge eval: no adapter found; attempting generic containerized evaluation",
                file=sys.stderr,
            )
        ...
        try:
            eval_summary = _run_containerized_eval(
                dataset_path=dataset_path,
                predictions_path=predictions_path,
                out_root=out_root,
                adapter_path=adapter_path,
                timeout_seconds=getattr(namespace, "timeout", 120),
                jobs=getattr(namespace, "jobs", 4),
    ...
    if command == "analyze":
                ...
                if prediction_count == 0:
                    ...
                    if adapter_path is not None:
                        ...
                        print(
                            ...
                            " attempting generic containerized evaluation",
                            file=sys.stderr,
                        )
                    print(
                        "repogauge analyze: running shared containerized eval"
                    ...
                    try:
                        ...
                        auto_eval_error = (
                            ...
                            "Inspect the evaluation error above, fix it, then rerun"
                            " `repogauge analyze`."
                        )
        ...
        if instance_results_path is None:
            hint = (
                "pass --dataset or drop --skip-eval so analyze can run evaluation"
    ...
    if command == "run":
        ...
        try:
            ...
            if containerized_workspace_solvers:
                runtime_context = ensure_container_runtime(
                    container_runtime=getattr(namespace, "container_runtime", "podman"),
                    container_host=getattr(namespace, "container_host", None),
                    log_prefix="repogauge run",
                )
            ...
            with execution_context as resolved_container_host:
                if runtime_context is not None:
                    if default_dataset_images_required and dataset_rows:
                        ...
                        emit_progress_line(
                            "repogauge run",
                            (
                                "preparing dataset environment images for "
                                f"{len(dataset_rows)} instances with jobs="
                                f"{getattr(namespace, 'jobs', 4)}"
                            ),
                        )
                        with temporary_environment(env_overrides):
                            ...
                            build_env_images(
                                ...
                                force_rebuild=False,
                                max_workers=getattr(namespace, "jobs", 4),
                                namespace=None,
                        ...
                        emit_progress_line(
                            "repogauge run",
                            "reused or built dataset environment images",
                        )
                ...
                scheduler = SolverScheduler(
                    config=SolverSchedulerConfig(
                        max_parallel_jobs=getattr(namespace, "jobs", 4),
