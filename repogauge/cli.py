...
def _build_parser() -> argparse.ArgumentParser:
    ...
    for name in ["mine", "review", "export", "eval", "run", "analyze", "train-router"]:
        ...
        if name == "eval":
            cmd.add_argument("--gold", action="store_true", help="Evaluate gold predictions (predictions.gold.jsonl).")
            cmd.add_argument("--predictions", help="Explicit predictions JSONL file (overrides --gold).")
            cmd.add_argument("--timeout", default=120, type=int, help="Per-instance pytest timeout in seconds.")
            cmd.add_argument("--adapter", help="Path to adapter_*.py generated by repogauge export. Auto-discovered when omitted.")
...
def _run_command(namespace: argparse.Namespace) -> int:
    ...
    if command == "export":
        ...
        try:
            ...
            if export_summary.get("ready_count", 0) > 0:
                ...
                mat_rows = [
                    json.loads(ln) for ln in mat_path.read_text(encoding="utf-8").splitlines() if ln.strip()
                ]
                if mat_rows:
            ...
            for profile_candidate in [
                ...
                if profile_candidate.exists():
                    try:
                        profile_data = json.loads(profile_candidate.read_text(encoding="utf-8"))
                        adapter_env_plan = profile_data.get("environment_plan", {})
                    except Exception:
                        pass
                    break
            if adapter_repo_name:
                adapter_result = generate_adapter(adapter_repo_name, adapter_env_plan, out_root=out_root)
    ...
    if command == "eval":
        if not namespace.path:
            ...
            manifest.mark_step("finish", ManifestStepStatus.FAILED, ended_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat() + "Z")
            manifest.write(manifest_path)
            log_event({"event": "command.finish", "command": command, "status": manifest.status, "timestamp": manifest.ended_at, "error": "missing dataset path"}, events_path)
            return 1
        ...
        if not dataset_path.exists():
            manifest.finish(status="failed", metadata={"reason": "dataset_not_found", "path": str(dataset_path)})
            manifest.mark_step("finish", ManifestStepStatus.FAILED, ended_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat() + "Z")
            manifest.write(manifest_path)
            log_event({"event": "command.finish", "command": command, "status": manifest.status, "timestamp": manifest.ended_at, "error": f"dataset not found: {dataset_path}"}, events_path)
            return 1

        if not predictions_path.exists():
            manifest.finish(status="failed", metadata={"reason": "predictions_not_found", "path": str(predictions_path)})
            manifest.mark_step("finish", ManifestStepStatus.FAILED, ended_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat() + "Z")
            manifest.write(manifest_path)
            log_event({"event": "command.finish", "command": command, "status": manifest.status, "timestamp": manifest.ended_at, "error": f"predictions not found: {predictions_path}"}, events_path)
            return 1
        ...
        if getattr(namespace, "adapter", None):
            ...
            if not adapter_path.exists():
                manifest.finish(status="failed", metadata={"reason": "adapter_not_found", "path": str(adapter_path)})
                manifest.mark_step("finish", ManifestStepStatus.FAILED, ended_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat() + "Z")
                manifest.write(manifest_path)
                log_event({"event": "command.finish", "command": command, "status": manifest.status, "timestamp": manifest.ended_at, "error": f"adapter not found: {adapter_path}"}, events_path)
                print(f"repogauge eval: error: adapter not found: {adapter_path}", file=sys.stderr)
                print("repogauge eval: hint: run `repogauge export` first to generate the adapter", file=sys.stderr)
                return 1
        else:
        ...
        if adapter_path is not None:
            try:
                ...
                _mod_spec = _ilu.spec_from_file_location("_repogauge_adapter", adapter_path)
                ...
                print(f"repogauge eval: warning: could not load adapter {adapter_path}: {exc}", file=sys.stderr)
                print("repogauge eval: hint: re-run `repogauge export` to regenerate the adapter", file=sys.stderr)
        else:
            print("repogauge eval: no adapter found; run `repogauge export` to generate one (using default harness settings)", file=sys.stderr)
        ...
        try:
            ...
            manifest.mark_step("inspect", ManifestStepStatus.SUCCEEDED)
            manifest.mark_step("execute", ManifestStepStatus.SUCCEEDED, ended_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat() + "Z")
            manifest.artifact_paths["validation"] = eval_summary["validation_path"]
            manifest.metadata["eval"] = {k: v for k, v in eval_summary.items() if k != "validation_path"}
            manifest.mark_step("finish", ManifestStepStatus.SUCCEEDED, ended_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat() + "Z")
            manifest.finish(status="succeeded", metadata={"reason": "eval_complete"})
            manifest.write(manifest_path)
            log_event({"event": "command.finish", "command": command, "status": manifest.status, "timestamp": manifest.ended_at}, events_path)
            total = eval_summary["total"]
            resolved = eval_summary["resolved"]
            rate = eval_summary["resolve_rate"]
            print(f"repogauge eval: {resolved}/{total} resolved ({rate:.1%})", file=sys.stderr)
            return 0
        except Exception as exc:
            manifest.mark_step("inspect", ManifestStepStatus.FAILED, ended_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat() + "Z")
            manifest.mark_step("execute", ManifestStepStatus.SKIPPED)
            manifest.finish(status="failed", metadata={"reason": "eval_failed", "error": str(exc)})
            manifest.mark_step("finish", ManifestStepStatus.FAILED, ended_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat() + "Z")
            manifest.write(manifest_path)
            log_event({"event": "command.finish", "command": command, "status": manifest.status, "timestamp": manifest.ended_at, "error": str(exc)}, events_path)
            return 1
