...
def score_scan_commit(
    *,
    ...
    score, score_breakdown = _apply_positional_scores(
        commit_subject, commit_body, diff, metadata, score, score_breakdown
    )
    score, score_breakdown = _apply_penalties(diff, metadata, score, score_breakdown)

    band = _decision_band(score)
    return ScoredCommit(
        score=score, decision_band=band, score_breakdown=score_breakdown
    )
...
def _is_docs_only(metadata: dict[str, Any]) -> bool:
    ...
    return not (has_prod or has_test) and (
        has_docs or has_config or has_test_support or has_unknown
    )
...
def _is_generated_vendor_only(metadata: dict[str, Any]) -> bool:
    ...
    if not has_vendor:
        return False
    return not (
        has_prod or has_test or _row_metadata_int(metadata, "n_unknown_files") > 0
    )
...
def _apply_positional_scores(
    ...
    if (
        _row_metadata_int(metadata, "n_prod_files") > 0
        and _row_metadata_int(metadata, "n_test_files") > 0
    ):
        score += 4
